...
`Bruce` 是一个本地优先的 macOS 菜单栏研发活动看板。它把分散在本机 AI Agent 会话和本地工具数据库中的研发活动汇总到一个原生应用中, 为开发者提供:
...
- `BruceApp`: 依赖装配、菜单栏场景、设置窗口和生命周期入口。
...
```javascript
window.__BruceUpdate(artifact, state)
```
...
配置位于用户级 Application Support 的 `Bruce/config` 下。目录权限为 `0700`, 文件权限为 `0600`, 写入采用同步、重读校验和原子替换。
...
Artifact 快照位于用户级 Application Support 的 `Bruce/snapshots` 下, metadata 位于 `Bruce/metadata` 下。快照可能包含个人活动、模型名称和项目名称, 必须按私有本机数据处理, 不得提交到 Git。
...
| 设计领域 | 主要代码 |
|---|---|
| 应用入口与依赖装配 | `macos/BruceApp/Sources/BruceApp/BruceApp.swift`, `ApplicationBootstrap.swift` |
| 菜单栏标签与面板 | `macos/BruceApp/Sources/BruceApp/MenuBarViews.swift` |
| 设置与授权 UI | `macos/BruceApp/Sources/BruceApp/SettingsView.swift` |
| UI 状态模型 | `macos/BruceApp/Sources/BruceAppCore/AppModel.swift` |
| 面板卡片组件 | `macos/BruceApp/Sources/BruceApp/Views/` (UsageHeroCard / SubscriptionCard / HourlyLineCard / PanelCardContainer) |
| 主题与玻璃样式 | `macos/BruceApp/Sources/BruceApp/GlassTheme.swift` |
| Onboarding 协调 | `macos/BruceApp/Sources/BruceApp/OnboardingCoordinator.swift` |
| 本机扫描与就绪度 | `macos/BruceApp/Sources/BruceOnboardingCore/` |
| 调度 | `macos/BruceApp/Sources/BruceAppCore/RefreshScheduler.swift` |
| Collector 进程 | `macos/BruceApp/Sources/BruceAppCore/CollectorRunner.swift` |
| 快照与校验 | `macos/BruceApp/Sources/BruceAppCore/ArtifactStore.swift`, `ArtifactModels.swift` |
...
```text
BruceOnboardingCore <- BruceAppCore <- BruceApp
                         ^
                         └─ App Core Harnesses
```
...
1. 创建空的 `BruceAppCore` target 并让 `BruceApp` 建立依赖。
...
```text
Bruce-diagnostics-YYYYMMDD-HHmmss.zip
```
