...
Bruce 当前通过 `CollectorRunner -> Python -> bridge/run_bridge.py -> collector` 完成采集. 该边界已经承担 stdin/stdout Bridge v1、超时、取消、`runId`、artifact 校验和凭证更新返回, Swift 的 `RefreshScheduler` 与 `RefreshExecutionPipeline` 也已经完成去重、scope 和 Codex recovery 逻辑.

开发文档 014 已确定生产实现迁移到 `rust/Bruce-collector/`, 采用独立进程而不是 Swift/Rust 双向 FFI. 本次实现还必须解决 182 天窗口下历史文件重复扫描、SQLite 重复读取、外部额度请求串行和 Python 中间对象造成的刷新耗时与资源问题.
...
- Preview 可保留 Python 对照; Release 不包含 Python fallback.

## Goals / Non-Goals

**Goals:**

- 先以脱敏 fixture 和基线指标冻结 Python 行为.
...
- 使用 Python/Rust differential parity、资源 benchmark、secret scan、Swift Runner harness 和 Release 包扫描作为阶段门禁.
...
### 5. Preview 双实现, Release 单 Rust

Swift 增加 `CollectorExecutable` seam, 提供 `RustBinaryAdapter` 和 Preview-only `PythonPreviewAdapter`. 对照模式只展示 primary 结果, 对副本做脱敏 canonical diff, Rust 失败时仅 Preview 可 fallback. Release manifest 不包含 Python runtime/source/bridge/fallback.
...
- [Parity 差异] Python 隐式类型转换、排序、四舍五入或错误分类可能在 Rust 中改变 → 先生成 golden fixture, 严格比较 artifact、status、diagnostics、updates/challenges, 运行时字段只做显式归一化.
...
- [迁移周期过长] Provider 协议风险高 → 先做纯逻辑和 local vertical slice, 再做 read-only provider, 最后做 OAuth refresh, 每阶段可回退 Python Preview.

## Migration Plan

1. Phase 0: 给 Python Bridge 增加仅诊断用途的 phase/resource metrics, 建立脱敏 request/response/artifact/provider/local fixtures 和 benchmark script.
2. Phase 1: 创建 Cargo workspace、Bridge skeleton、输入大小限制、stdout 单 envelope、退出码和 Runner harness.
3. Phase 2: 迁移 domain/time/status/artifact/aggregate, 建立 Python/Rust differential harness.
...
9. Phase 9: 生成签名 universal Release, 移除 Python runtime/source/fallback, 做安装/升级/回滚验收.

回滚顺序:

- 删除或失效 cache, 回到完整 Rust 扫描.
- Preview 选择 Python adapter.
