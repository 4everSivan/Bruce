...
- [x] 1.1 Add opt-in Bridge phase/resource metrics without changing artifact output or credential behavior
...
- [x] 1.4 Add canonical result normalization and secret scanning for differential fixtures
...
- [x] 3.3 Build canonical fixture runner with strict artifact/status/diagnostic comparison
...
- [x] 6.1 Add `CollectorExecutable` seam with Rust binary only
- [x] 6.2 Update runtime readiness, diagnostics, and settings to fail closed on missing Rust runtime
- [x] 6.3 Update Preview/runtime manifests and build scripts to use the same Rust artifact source
- [x] 6.4 Add Release manifest/package checks that reject legacy runtime/source, secrets, data, fixtures, and wrong-architecture binaries
...
- [x] 7.1 Run the full cold/warm/account-scale Rust benchmark matrix and compare target metrics
...
- [x] 7.4 Run Rust, Swift, package security, and standard local verification suites
...
- [x] 8.6 Remove the legacy script Collector, Bridge, tests, benchmarks, configuration, and CI jobs; verify Rust-only source and bundle scans
