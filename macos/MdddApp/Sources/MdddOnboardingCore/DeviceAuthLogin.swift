...
public struct DeviceAuthorization: Equatable, Sendable {
    /// 服务端下发的设备凭证 (GitHub device_code / Codex device_auth_id).
...
/// GitHub 设备码登录, 与 `gh auth login --web` 同源:
/// 使用 gh 官方 OAuth App (GitHub CLI) 的公开 client_id 走 device flow,
/// 拿到的 token 由调用方经 `gh auth login --with-token` 写回 gh 登录态.
/// 网络层可注入, 仅由用户点击触发.
public struct GitHubDeviceFlow: Sendable {
    /// gh 官方 OAuth App (GitHub CLI) 的公开 client_id.
    public static let clientID = "178c6fc778ccc68e1d6a"
    /// 与 gh 浏览器登录一致的授权范围.
    public static let scope = "repo read:org gist workflow"
    public static let deviceCodeURL = URL(
        string: "https://github.com/login/device/code"
    )!
    public static let accessTokenURL = URL(
    ...
    /// 设备码申请请求: POST form client_id + scope.
    ...
    /// 解析设备码申请响应: 200 且含 device_code / user_code 才成功 (fail-closed).
    public static func parseStartResponse(
        ...
        let deviceCode = (dict["device_code"] as? String ?? "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let userCode = (dict["user_code"] as? String ?? "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        guard !deviceCode.isEmpty, !userCode.isEmpty else {
            return .failure(.invalidResponse("缺少 device_code 或 user_code"))
        }
        let verificationURI = (dict["verification_uri"] as? String ?? "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let verificationURL = URL(string: verificationURI) ?? self.verificationURL
        let interval = (dict["interval"] as? NSNumber)?.doubleValue ?? defaultInterval
        let expiresIn = (dict["expires_in"] as? NSNumber)?.doubleValue ?? 900
    ...
    /// 轮询请求: POST form client_id + device_code + device_code grant.
    public static func pollRequest(deviceCode: String) -> URLRequest {
        var request = URLRequest(
            url: accessTokenURL,
            timeoutInterval: DeviceFlowHTTP.requestTimeout
        )
    ...
    /// 解析轮询响应. GitHub 在 200 下用 error 字段表示 pending/slow_down/拒绝/过期.
    public static func parsePollResponse(
        ...
        if let token = (dict["access_token"] as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !token.isEmpty {
            return .success(.authorized(accessToken: token))
        }
        switch dict["error"] as? String {
        ...
        default:
            let detail = (dict["error"] as? String) ?? "缺少 access_token"
            return .failure(.invalidResponse(detail))
        }
    }
    ...
    /// 按服务端建议间隔轮询直到授权, 拒绝或过期; 支持任务取消.
    /// slow_down 时按协议把间隔上调 5 秒.
    public func pollUntilAuthorized(
        _ authorization: DeviceAuthorization,
        session: (any URLSessionProtocol)? = nil
    ) async -> Result<String, DeviceAuthError> {
        let activeSession = session ?? DeviceFlowHTTP.defaultSession()
        var interval = max(authorization.interval, 0)
        ...
        while true {
            ...
            do {
                (data, statusCode) = try await DeviceFlowHTTP.send(
                    Self.pollRequest(deviceCode: authorization.deviceCode),
            ...
            case .success(.pending):
                continue
            case .success(.slowDown):
                interval += 5
                continue
            case .success(.authorized(let accessToken)):
    ...
    /// 完整状态机: 申请 -> onAuthorization 回调展示验证码 -> 轮询 -> 返回 token.
    public func login(
        session: (any URLSessionProtocol)? = nil,
        ...
        case .success(let authorization):
            ...
            return await pollUntilAuthorized(authorization, session: session)
        }
    }
}
