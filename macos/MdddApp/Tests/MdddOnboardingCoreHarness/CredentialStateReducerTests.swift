...
enum CredentialStateReducerTests {
    ...
    private static func connectedState(
        ...
        origin: CodexCredentialOrigin = .mddd
    ...
    private static func applyLoginIncrementsRevisionAndClearsBlocked() throws {
        ...
        try reducerExpect(applied.state.credentialOrigin == .mddd, "登录 origin=mddd")
    ...
    private static func applyRefreshResultTable() throws {
        let base = connectedState(email: "u@x.com", origin: .mddd)
        ...
        try reducerExpect(success.state.credentialOrigin == .mddd, "success 保留 origin")
